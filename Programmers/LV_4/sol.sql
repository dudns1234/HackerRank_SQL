...
-- 5월 식품들의 총매출 조회하기 (AMOUNT 곱하기)
SELECT FD.PRODUCT_ID, PRODUCT_NAME, SUM(AMOUNT*PRICE) TOTAL_SALES
FROM FOOD_PRODUCT FD JOIN (SELECT * FROM FOOD_ORDER WHERE DATE_FORMAT(PRODUCE_DATE,'%Y-%m') = '2022-05') FO ON FD.PRODUCT_ID = FO.PRODUCT_ID
GROUP BY FD.PRODUCT_ID
ORDER BY TOTAL_SALES DESC, PRODUCT_ID

SELECT FD.PRODUCT_ID, PRODUCT_NAME, SUM(AMOUNT*PRICE) TOTAL_SALES
FROM FOOD_PRODUCT FD, FOOD_ORDER FO
WHERE FD.PRODUCT_ID = FO.PRODUCT_ID AND DATE_FORMAT(PRODUCE_DATE,'%Y-%m') = '2022-05'
GROUP BY FD.PRODUCT_ID
ORDER BY TOTAL_SALES DESC, PRODUCT_ID

-- 식품분류별로 가장 비싼 식품의 정보 조회하기
...
-- 입양 시각 구하기(2) : 난이도 극상상상
WITH RECURSIVE CTE AS(
    SELECT 0 AS NUM # 초기값 설정
    UNION ALL
    SELECT NUM+1 FROM CTE
    WHERE NUM < 24 # 반복을 멈추는 조건
)

# SELECT * FROM CTE

SELECT NUM ,CASE
              WHEN HOUR IS NULL THEN 0
              ELSE COUNT(*)
            END COUNT  
FROM (SELECT *, HOUR(DATETIME) HOUR FROM ANIMAL_OUTS) AO RIGHT JOIN CTE ON CTE.NUM = AO.HOUR  
